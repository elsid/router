...
struct Action {
    ...
    template <class T>
    constexpr Action(std::string_view name, T&& f) : name(name), f(std::forward<T>(f)) {}
...
template <class F>
Action(std::string_view, F&& f) -> Action<std::decay_t<F>>;
...
template <class F>
struct ArgumentsNumber<Action<F>> : ArgumentsNumber<F> {};
...
struct Selector {
    ...
    template <class ... Ts>
    constexpr explicit Selector(std::string_view name, Ts&& ... actions)
        : name(name), actions(std::forward<Ts>(actions) ...) {}
    ...
    void find_action(std::string_view action_name, F&& f) const {
...
template <class ... Actions>
Selector(std::string_view, Actions&& ...) -> Selector<std::decay_t<Actions> ...>;

} // namespace

int main() {
    constexpr Selector dispatch("",
        Action("roll_dice", &roll_dice),
        Selector("spells",
            Action("add", &add_spell),
            Action("cost", [] (Spell spell) { spell_cost(spell); })
        ),
        Selector("wizards",
            Action("add", &add_wizard),
            Cast {},
            Action("learn", &learn),
            Action("channel", &channel),
            Action("mana", &wizard_mana)
        )
    );
