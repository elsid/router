...
std::error_code learn(Wizard wizard, Spell spell) {
    const auto wizard_it = state.wizards.find(wizard.name);
...
std::error_code add_spell(Spell spell, Mana cost) {
...
std::error_code add_wizard(Wizard wizard, Mana mana) {
    if (state.wizards.find(wizard.name) != state.wizards.end()) {
        return std::make_error_code(std::errc::invalid_argument);
    }
...
std::error_code channel(Wizard wizard, Mana mana) {
...
std::error_code wizard_mana(Wizard wizard) {
    const auto it = state.wizards.find(wizard.name);
    if (it == state.wizards.end()) {
        return std::make_error_code(std::errc::invalid_argument);
    }
...
std::error_code spell_cost(Spell spell) {
...
struct Cast {
    ...
    std::error_code operator ()(Wizard wizard, Spell spell) const {
        return cast(wizard, spell);
    }
};
...
int main() {
    constexpr Selector dispatch(
        ...
        Action(spells_tag, Selector(
            ...
            Action(spell_cost_tag, [] (Spell spell) { return spell_cost(spell); })
        )),
    ...
    for (std::string line; std::getline(std::cin, line);) {
        ...
        try {
            std::visit(PrintResult {}, dispatch(input));
